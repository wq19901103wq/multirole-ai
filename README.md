...
| **动态角色匹配** | ✅ 完成 | `PersonaGenerator` 根据话题分析生成最自然的讨论维度，避免硬套角色 |
...
传统多代理系统使用固定职业角色（如永远是"工程师"+"分析师"+"文案师"），这会导致非技术话题下角色强行套角度、抓不住重点，甚至为了"凑角色"而说些不相关的内容。

我们的解决方案：
- **取消固定职业角色**，改为基于话题本身推导出最自然的讨论维度
- **`PersonaGenerator`** 在有真实 LLM 时，会先调用模型分析话题最适合从哪 3-4 个角度讨论
- 每个角度就是一个 Agent，没有职业身份，只有**思考视角**

### 示例对比

**话题："当代年轻人的婚恋观"**
- ❌ 旧方案：规划师 + 工程师（硬套技术角度）+ 分析师（硬找数据）+ 文案师
- ✅ 新方案：社会变迁视角 + 代际差异视角 + 个体心理视角 + 文化表达视角

**话题："周末应该去哪里玩"**
- ❌ 旧方案：四个职业角色完全错位
- ✅ 新方案：预算性价比 + 体验丰富度 + 交通便利性（仅生成最相关的维度）

### 柔性约束

每个角色的 Prompt 都带有：
> "如果这个话题与你的角度关联不大，请不要强行套用专业术语，从普通人的常识逻辑出发，直接回应该议题的核心。"

### Fallback

当 LLM 不可用时，`PersonaGenerator` 会基于关键词做本地规则匹配，并补充通用思考维度（如"关键权衡"、"实际影响"），确保始终有 3-4 个自然角度。
