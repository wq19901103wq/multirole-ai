...
## 圆桌讨论模式（PersonaGenerator）

传统多代理系统最大的问题是**各自独白**：每个 Agent 被分配一个固定角度，然后轮流读稿，彼此之间没有真正的交流和碰撞。

我们的解决方案：
- **取消固定职业角色和预设角度**
- **4 个 Agent 都是平等的思考者**，没有"工程师"、"分析师"之类的身份标签
- **真正的对话式讨论**：后面的 Agent 能看到前面所有人的发言，可以自由地补充、质疑、反驳或综合
- **核心 Prompt 约束**：
  > "你可以赞同、补充、质疑或反驳前面发言者的观点。这是一个真实的讨论，不是各自独白。你的目标是推动讨论向更深入、更正确的方向发展。"

### 为什么这样更好？

**话题："人工智能是否会取代程序员"**
- ❌ 旧方案：A 从"技术角度"、B 从"经济角度"、C 从"社会角度"各说一段，互不相关
- ✅ 新方案：思考者一提出直觉判断 → 思考者二指出其中的隐含假设 → 思考者三补充一个反例 → 思考者四尝试综合出一个更 nuanced 的结论

**话题："周末应该去哪里玩"**
- ❌ 旧方案：四个职业角色完全错位
- ✅ 新方案：第一个 Agent 提议海边 → 第二个质疑交通成本 → 第三个补充附近有不错的民宿 → 第四个综合出一个可行方案

### 防漂移机制如何配合？

在自由的对话模式下，我们保留了 TopicAnchor 的两条核心约束：
1. **禁止跑题高频词**（"此外"、"值得一提的是"...）
2. **强制自评相关性**（每次发言最后必须给出 X/10 的相关性评分）

这样既保证了讨论的自由度，又防止了话题漂移。
